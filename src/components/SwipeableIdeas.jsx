...
export default function SwipeableIdeas() {
  ...
  const [viewedOutfits, setViewedOutfits] = useState(() => {
    const savedViewed = localStorage.getItem('viewedOutfits');
  ...
  useEffect(() => {
    ...
    localStorage.setItem('viewedOutfits', JSON.stringify(viewedOutfits));
  }, [currentIndex, viewedOutfits]);
  ...
  const handleDragEnd = () => {
    ...
    if (offset.x > threshold) {
      if (currentIndex < posts.length) {
        ...
        setViewedOutfits([...viewedOutfits, posts[currentIndex].id]);
  ...
  const handleClick = () => {
    if (!isDragging && currentIndex < posts.length) {
      navigate(`/outfit/${filteredPosts[currentIndex]?.id}`);

    }
  };

  const filteredPosts = posts.filter(post => !viewedOutfits.includes(post.id));
  ...
  console.log(`Current outfit ID: ${filteredPosts[currentIndex]?.id}`);
  ...
  return (
    <div className="flex flex-col items-center w-full min-h-screen bg-gray-50 p-8">
      ...
      >
        ...
        >
          <img
            src={filteredPosts[currentIndex]?.thumbnail}
            alt={`Outfit ${filteredPosts[currentIndex]?.id}`}
